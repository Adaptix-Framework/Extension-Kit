            ...
            { 
                ...
                if (retCode == ERROR_SUCCESS) 
                {
                    if(recursive)
                    {
                        ...
                        if(dwresult)
                        {
                            BeaconPrintf(CALLBACK_ERROR, "Could not open key %s\\%s\\%s: Error %lx", 
                                gHiveName, curitem->keypath, currentkeyname, dwresult);
                            dwresult = ERROR_SUCCESS;  // ← reset, don't poison final return
                        }
                        else {
                    ...
                    {
                        curKey = NULL;
                        // Don't propagate this open error — it's only for timestamp display
                        DWORD tsResult = ADVAPI32$RegOpenKeyExA(curitem->hreg, currentkeyname, 0, KEY_READ, &curKey);
                        if (curKey) {
                            Reg_KeyToTimestamp(curKey, stringDate);
                            ADVAPI32$RegCloseKey(curKey);  // ← also close it, was leaking
                            curKey = NULL;
                        }
                        internal_printf("%-24s %s\\%s\\%s\n", 
                            (tsResult == ERROR_SUCCESS) ? stringDate : "Unable to get time", 
...
) 
{
    ...
    // testing the args passed
    BeaconPrintf(CALLBACK_OUTPUT, "Hostname: %s, Hive: %s, Path: %s, Key: %s, Recursive: %d", (hostname) ? hostname : "NULL", gHiveName, path, (key) ? key : "NULL", recursive);
